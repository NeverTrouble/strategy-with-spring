...
@Slf4j
@Component
public class ExcelResolverContext {

    /**
     * 这里是 sheetName 对应的 解析器
     */
    private final Map<String, ExcelResolver> SHEET_NAME_TO_RESOLVER;
     ...
     */
    @Autowired
    private ExcelResolverContext(Map<String, ExcelResolver> excelResolverMap) {
        ...
        for (Map.Entry<String, ExcelResolver> entry : excelResolverMap.entrySet()) {

            // 这个值就是 @Service("双引号里的")
            // 我这里没有使用它, 而是在每个实现里加了一个 sheetName 的字段来标识
            // String beanName = entry.getKey();

            ExcelResolver resolver = entry.getValue();
    ...
    public void resolve(String sheetName, Object data) {
        ...
        ExcelResolver excelResover = SHEET_NAME_TO_RESOLVER.get(sheetName);
